...
class UserController extends Controller
{
    ...
    public function create()
    {
        ...
        try {
            ...
            return view($this->view . 'create', compact('companyDetail', 'roles', 'employeeCode', 'bsEnabled', 'countries'));
        } catch (Exception $exception) {
            return redirect()->back()->with('danger', $exception->getMessage());
        }
    }
    ...
    public function store(UserCreateRequest $request, UserAccountRequest $accountRequest, UserLeaveTypeRequest $leaveRequest)
    {
        ...
        try {
            ...
            if ($request->hasFile('visa_copies')) {
                foreach ($request->file('visa_copies') as $file) {
                    $fileName = $this->userRepo->uploadImage($file, VisaAttachment::UPLOAD_PATH);
                    VisaAttachment::create([
                        'user_id' => $user->id,
                        'file_path' => VisaAttachment::UPLOAD_PATH,
                        'file_name' => $fileName,
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            return view($this->view . 'edit', compact('companyDetail', 'roles', 'userDetail', 'leaveTypes', 'employeeLeaveTypes', 'bsEnabled','filteredSupervisor','filteredPosts', 'countries'));
        } catch (Exception $exception) {
...
//    }
    ...
    public function deletePassportAttachment($id)
    {
        $this->authorize('edit_employee'); // Assuming this permission is appropriate
        try {
            $attachment = \App\Models\PassportAttachment::findOrFail($id);

            // Delete file from storage
            $this->userRepo->removeImage(\App\Models\PassportAttachment::UPLOAD_PATH, $attachment->file_name);

            // Delete record from database
            $attachment->delete();

            return response()->json(['success' => true, 'message' => __('index.passport_copy_deleted')]);
        } catch (Exception $e) {
            \Illuminate\Support\Facades\Log::error("Failed to delete passport attachment: " . $e->getMessage());
            return response()->json(['success' => false, 'message' => __('index.failed_to_delete_passport_copy')], 500);
        }
    }

    public function deleteVisaAttachment($id)
    {
        $this->authorize('edit_employee'); // Assuming this permission is appropriate
        try {
            $attachment = VisaAttachment::findOrFail($id);

            // Delete file from storage
            $this->userRepo->removeImage(VisaAttachment::UPLOAD_PATH, $attachment->file_name);

            // Delete record from database
            $attachment->delete();

            return response()->json(['success' => true, 'message' => __('index.visa_copy_deleted')]);
        } catch (Exception $e) {
            \Illuminate\Support\Facades\Log::error("Failed to delete visa attachment: " . $e->getMessage());
            return response()->json(['success' => false, 'message' => __('index.failed_to_delete_visa_copy')], 500);
        }
    }
}
